...
function Sidebar({
  ...
  const [resolvedSide, setResolvedSide] = React.useState<'left' | 'right'>(initialResolvedSide)
  ...
  React.useEffect(() => {
    ...
    if (typeof document !== 'undefined') {
      setResolvedSide(document.documentElement?.dir === 'rtl' ? 'right' : 'left')
    }
  }, [side, dir])
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className="sr-only">{t('general.toggleSidebar')}</span>
    </Button>
  )
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  const { toggleSidebar } = useSidebar()
  const t = useTranslations('common')
  const toggleLabel = t('general.toggleSidebar')
